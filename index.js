...
function get20s(array){
  ...
  for(let i =0; i< array.length; i++) {
    ...
    const died = parseInt(yearsArray[yearsArray.length - 1]);
...
function addArtist(newArtistObject){
  artists.push(newArtistObject);
  return artists;
}

console.log(addArtist({id: 20, name: 'James Lundin', years: '1990 - 2020', genre: 'Web Design', nationality: 'Irish', bio: 'Sed aliquet eget dui eget suscipit. Morbi id blandit orci. Nullam ut consequat arcu. Suspendisse quis eleifend risus. Nullam rhoncus nec nibh nec mollis. Quisque suscipit consequat risus at hendrerit. In ornare vehicula arcu quis pharetra.' }));
...
function lotsOfArt(array){
  const newArray = [];
  for(let i = 0; i < array.length; i++) {
    if(array[i].hasOwnProperty('paintings')) {
...
function getHTML(array){
  let newString = '';
  for(let i = 0; i < array.length; i++) {
    let imgNameArray = 'assets/' + array[i].name.replace(" ", "-") + '.jpg';
    newString +=
      `<div id="artist"><div class="image"><img src="${imgNameArray}"/></div><div class="name"><a href="${array[i].wikipedia}"> ${array[i].name}</a></div><div class="bio">${array[i].bio}</div>\n</div>`;
  }
  return newString;
}
...
let artists19002000 = artists.filter((item) => {
  const yearsArray = item.years.split(" ");
  const born = parseInt(yearsArray[0]);
  const died = parseInt(yearsArray[yearsArray.length - 1]);
  if(born >= 1900 && died < 2000) {
    return true;
